...
public class SchoolClassesController {
	...
	public String displayChangeSchoolClassForm(@RequestParam(value="schoolClassId", required=false) String schoolClassId,
			Model model, HttpSession session) {
		if (session.getAttribute("userLogin") == null)
			return "redirect:/Login";
		model.addAttribute("editSchoolClass", DatabaseConnector.getInstance().getSchoolClassObjById(schoolClassId));
		model.addAttribute("schools", DatabaseConnector.getInstance().getSchools());
		return "schoolClassChangeForm";
	}

	@RequestMapping(value = "/UpdateSchoolClass", method = RequestMethod.POST)
	public String updateSchoolClass(@RequestParam(value = "classId", required = false) String schoolClassId,
			@RequestParam(value = "oldSchool", required = false) String oldSchoolId,
			@RequestParam(value = "trueSchool", required = false) String trueSchoolId,
		...
		SchoolClass schoolClass = DatabaseConnector.getInstance().getSchoolClassObjById(schoolClassId);
		schoolClass.setStartYear(Integer.valueOf(startYear));
    	schoolClass.setCurrentYear(Integer.valueOf(currentYear));
    	schoolClass.setProfile(profile);
